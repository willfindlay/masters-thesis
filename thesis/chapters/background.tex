...
owns, simply by restricting any access to its own \gls{uid}; secondly, the resulting process no
longer needs to run under the same \gls{uid} as its parent. This effectively limits the amount
...
have been implemented in other systems such as FreeBSD's Capsicum~\todo{CITE} and the
CHERI architecture~\cite{watson2015_cheri, davis2019_cheriabi}. These are discussed in
more detail in \Cref{ss:syscall-filtering}.
...
marked as setuid root. Confused deputy attacks~\cite{hardy1988_confused_deputy,
