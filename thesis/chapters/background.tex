...
\Cref{s:confinement-definition} provides a definition of the term
...
% Historically, confinement has been enforced at various layers of the software stack.
% Operating systems generally expose a set of \textit{confinement primitives} which

% We define \textit{confinement} as the restriction of some unit of computation
% $\mathcal{C}$ (e.g.\ a process or container) such that the set of all allowed actions
% $\mathcal{A}_{allowed} \subseteq \mathcal{A}_{possible}$ is the minimal subset required
% for $\mathcal{C}$ to function normally. In particular, we are concerned with limiting
% access to security sensitive resources and privileged operations which could affect global
% system state. An \textit{enforcement engine} mediates access to such resources and
% privileged operations by enforcing a \textit{confinement policy}. As defenders, our goal
% is to limit the ability of a misbehaving $\mathcal{C}$ (for example, having been
% compromised by an attacker) to damage the rest of the system.
...
\Cref{c:confinement-problem} discusses these two models in more detail. The rest of this
...
%and the CHERI architecture~\cite{watson2015_cheri, davis2019_cheriabi}.
...
% While containers are more lightweight than virtual machines, they are necessarily less
