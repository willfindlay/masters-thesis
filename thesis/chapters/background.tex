...
approach is similar in spirit to taint tracking mechanisms~\todo{CITE}
...
\gls{lsm} hooks~\todo{CITE}. \Cref{fig:lsm} depicts the basic \gls{lsm} architecture.
...
particular, \textit{system call filtering} is a widely-used~\cite{} technique for application
...
overhead (on the order of \todo{WHAT PERCENT}) and propensity to introduce undefined
behaviour when tracing even moderately complex software~\todo{CITE}. Due to its invasive
nature, a tracer process must either be the direct parent of a tracee or must have
sufficient privileges to trace the child process\,---\,on Linux, this translates to either
the \texttt{CAP\_SYS\_PTRACE} capability or the all-encompassing \texttt{CAP\_SYS\_ADMIN}.
...
\todo{Maybe other examples here}

\subsubsection*{OpenBSD Pledge and Unveil}
\label{sss:pledge}

OpenBSD's \texttt{pledge(2)} and \texttt{unveil(2)} system calls

\begin{inprogress}
\begin{itemize}
  \item pledge(2)~\cite{pledge}
  \begin{itemize}
    \item Takes a list of \textit{promises}, high-level privileges that are similar in
          spirit to Janus~\cite{goldberg96_janus, wagner1999_janus}
    \item \texttt{stdio} promise allows 69 total system calls, enabling any stdio functionality from libc
    \item others like \texttt{chown} are more conservative, allowing only one (albeit very powerful) system call
    \item In addition to promises, a call to pledge may also specify a set of promises to
    be applied after an \texttt{execve}, enabling a wrapper application to sandbox another application for instance
    \item Subsequent calls to pledge take the union of new promises and existing promises, preventing privilege escalation
    \item Pledge has been criticized for being overly-permissive~\cite{anderson2017_comparison}
  \end{itemize}

  \item unveil(2)~\cite{unveil, corbet2018_unveil}
  \begin{itemize}
    \item Like pledge, the unveil system call can be used to make a promise about how the process will behave
    \item Unlike pledge, unveil operates on specific filesystem paths, applying some
    combination of four permissions:
    \begin{itemize}
      \item read
      \item write
      \item execute
      \item create/delete
    \end{itemize}
    \item An unveil on a directory unveils all inodes under that directory, recursively
    \item Symlinks could be a concern here
  \end{itemize}
\end{itemize}
\end{inprogress}
...
\begin{inprogress}
  \begin{itemize}
    \item \texttt{seccomp(2)} \todo{CITATION FOR SECCOMP} allows a Linux process to
    confine itself by restricting the system calls it can make
    \item Originally, seccomp was highly limited, restricting a process to only four system calls:
    \begin{itemize}
      \item read
      \item write
      \item sigreturn (to handle signals)
      \item exit (to quit)
    \end{itemize}
  \end{itemize}
\end{inprogress}
