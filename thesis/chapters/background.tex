...
section in favour of \Cref{s:security-extensions} which examines more advanced security
...
consistently. A similar mechanism, \gls{kaslr}, protects the kernel. \todo{Maybe talk
about grsecruity here?}
...
than virtual machines, but at the cost of weaker isolation. \Cref{fig:virt} depicts the
...
A general threat model for container security consists of a number of attack vectors, each
...
external process. \Cref{fig:containersec} depicts a high-level overview of this threat
model.
...
\gls{lxc}~\todo{CITE} is a container runtime for Linux that directly exposes low-level
...
\todo{DTrace here, then talk about how it inspired later eBPF tooling (bpftrace, etc) and
the motivation behind eBPF}
