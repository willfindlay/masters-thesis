...
\todo{Describe virtual memory and memory protection}

\subsection{The Reference Monitor}%
\label{ss:refmon}

\todo{Describe reference monitor}

\begin{inprogress}
  \begin{itemize}
    \item First proposed by Anderson along with access matrix~\cite{anderson1972_report}
    \item Refer back to Paul and Jaeger book~\cite{van_oorschot2020_tools_jewels, jaeger2008_os_security}
  \end{itemize}
\end{inprogress}
...
logging in. While necessary under the Unix \gls{dac} model, setuid and setgid binaries have long
...
\todo{This probably belongs in \enquote{The Confinement Problem}?}

\begin{inprogress}
  \begin{itemize}
    \item \gls{dac} assumes that all processes are benign and contain no exploitable vulnerabilities (dangerous and invalid assumption in practice)
    ...
    \item Other attacks against \gls{dac}?
  \end{itemize}
\end{inprogress}
...
must be addressed. Many have turned to mandatory access control~\todo{CITE}
(c.f.~\Cref{ss:mac}) to solve the fundamental issues in \gls{dac}, while others have proposed
improvements or alternative schemes for implementing discretionary access control. This
subsection focuses on the latter.

\begin{inprogress}
  \begin{itemize}
    \item Mao \etal~\cite{mao2009_trojan_resistant_dac} proposed a new \gls{dac} model (IFEDAC) that is
...
\begin{figure}[tbp]
  ...
  \gls{bpf}, \gls{ebpf} programs are JIT compiled to the native instruction set, share data using
...
\subsubsection*{\gls{ebpf} Program Types and Use Cases}
...
\caption{A selection of relevant \gls{ebpf} program types for \bpfbox{} and \bpfcontain{}.}%
...
\subsubsection*{LSM Probes: Making Security Decisions with \gls{ebpf}}
...
userspace applications. Like \gls{ebpf} programs, maps can be pinned to \textit{bpffs} to
...
\caption{A selection of relevant \gls{ebpf} map types for \bpfbox{} and \bpfcontain{}.}%
\label{tab:map-types}\\
  \toprule
...
\subsection{Comparing \gls{ebpf} with Loadable Kernel Modules}%
