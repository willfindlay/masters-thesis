...
\Cref{ss:mac}. Classically, Unix-like systems have implemented discretionary access
...
(c.f.~\Cref{ss:mac}) to solve the fundamental issues in DAC, while others have proposed
...
In essence, classic BPF is a \textit{register virtual machine} designed to take packets as
...
\texttt{seccomp(2)} system call~\todo{CITE Anderson} was enhanced to include classic BPF
...
Once loaded into the kernel, eBPF programs are represented as BPF objects, each with its
...
programs to the Linux kernel's LSM security hooks (c.f.~\Cref{ss:mac}). Unlike
...
use of several eBPF map types, which are summarized in \Cref{tab:map-types}.
...
\bpfcontain{} (c.f.~\Cref{c:bpfcontain}) leverages libbpf and CO-RE through libbpf-rs, the
...
At a first approximation, eBPF differs from kernel modules in the following meaningful ways~\cite{gregg2019_bpf}:
\begin{enumerate}
...
In summary, eBPF is useful for observability use cases, or cases in which the functional
