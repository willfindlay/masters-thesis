...
section in favour of \Cref{s:extensions-bg} which examines more advanced security
...
than virtual machines, but at the cost of weaker isolation. \Cref{fig:virt} and
\Cref{tab:virt-comparison} illustrate the major differences between containers and virtual
machines.
...
Linux containers have three broad goals. In order of perceived importance and prioritization in existing
...
\begin{enumerate}
  \item \textbf{Dependency Management / Reproducibility.}
    ...
    date~\todo{CITE}. Docker Hub~\todo{CITE} allows self-contained container images to be
  ...
  \item \textbf{Virtualization.}
    ...
    filesystems~\todo{CITE} combined with the mount namespace aid containers one-way
...
Although hypervisor escapes are not unheard of~\todo{CITE}, they are far less common than
...
\todo{Outline threat model for containers, rework this and update the figure}
The threat model for container security consists of a few different attack surfaces and
corresponding attack vectors. \Cref{fig:containersec} depicts a high-level overview of
...
with a general focus on observability rather than policy enforcement. Tracee~\todo{CITE}
...
for further analysis. Cilium~\todo{CITE} is a popular security daemon for the Kubernetes
