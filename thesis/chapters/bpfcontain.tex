...
\bpfcontain{} policy in more detail in \Cref{s:bpfcontain-policy}.
...
policy file as required. \Cref{ss:bpfcontain-default} examines the implementation of
...
\bpfcontain{} supports four distinct policy decisions for every security-sensitive
...
(\Cref{ss:bpfcontain-default}). This process is depicted in full in
\Cref{fig:bpfcontain-enforcement} on page \pageref{fig:bpfcontain-enforcement}.
...
Aside from policy metadata, the policy is divided into four sections: allow, deny, taint,
...
the rego~\todo{CITE} policy framework, both of which are encoded in JSON\@.
...
defining exceptions to this protection boundary. \Cref{tab:bpfcontain-comparison} presents
...
%   \toprule
%                  & Userspace        & Kernelspace & Dependencies & Default Policy & Policy Language \\
%   \midrule
%   \bpfbox{}      & Python + bcc     & bcc + LLVM  & $>800\text{MiB}$ & Deny & Custom DSL \\
%   \bpfcontain{}  & Rust + libbpf-rs & \glsentryshort{bpf} \glsentryshort{core} & $<5\text{MiB}$ & Container Boundary & YAML/JSON/TOML \\
%   \bottomrule
% \end{longtable}
% \endgroup
...
\label{tab:bpfcontain-comparison}\\
  \toprule
                            & \bpfbox{}        & \bpfcontain{} \\
  \midrule
  Userspace                 & Python + bcc     & Rust + libbpf-rs\\
  Kernelspace               & bcc + LLVM       & \glsentryshort{bpf} \glsentryshort{core} \\
