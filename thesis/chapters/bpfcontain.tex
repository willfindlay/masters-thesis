...
\section{\bpfbox{} Limitations and the Transition Toward \bpfcontain{}}%
...
% as a confinement mechanism. \Cref{s:bpfcontain-improvements} of \Cref{c:bpfcontain} offers
% some insights into how \bpfcontain{} addresses these limitations.

\todo{Introductory paragraph, explaining that \bpfbox{} has some fundamental limitations
and that \bpfcontain{} was created to address them. \enquote{We enumerate these
...
\todo{Differences that are motivating and differences in how it came out. Talk about
granularity of policies: with bpfbox it's the executable, with bpfcontain it's the
container. Things like Docker integration wouldn't be possible with bpfbox for a number of
reasons.}

\todo{Process confinement problem: nobody has really phrased it as the process group
confinement problem... Almost ignored. Confining a single process is very different from
confining a process group, bpfbox does the former, bpfcontain does the latter. Developers
can do a good job at confining their own binaries... In practice people don't just run one
binary.}

\todo{Explain the limitations of \bpfbox{} and in a couple of paragraphs explain how we address this}
\todo{bpfbox really doesn't talk about groups of processes}
\todo{Active patches to the kernel that enabled new functionality, so it follows that
later on in the thesis we are able to more than we could before}

\todo{Forward reference to the end part}
...
there are a few distinct differences. We enumerate these differences as follows.  Maps
types are outlined in \textbf{\green{green}} and program types are outlined in
\textbf{\purple{purple}}.  The reader is encouraged to revisit
\Cref{ss:bpfbox-architecture} of \Cref{c:bpfbox} for a comparison between the two systems.
\begin{itemize}
  ...
  is a dynamically-allocated and garbage-collected per-structure storage blob. \bpfcontain{}
  leverages these for more memory-efficient storage of per-task and per-inode state.
