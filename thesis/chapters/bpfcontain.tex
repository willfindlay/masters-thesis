...
serialization crate for Rust. \todo{Explain the design choice that motivated us to move
away from a DSL... We want to have people design policies and so the tooling around the
policy is kind of important.}

Serde leverages Rust's powerful type system and procedural
...
\subsubsection{Improving Default Policy}%
\label{sss:bpfcontain-improving-default}

\todo{Talk about our desire to enforce overlayfs policy, if the implementation ends up
taking too long. Also: Need to go over the rest of this chapter and remove references to
namespace file policy.}
