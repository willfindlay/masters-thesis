...
\Cref{s:cp-design} in \Cref{c:confinement-problem}, our primary design goals are
...
In userspace, a privileged daemon loads \bpfbox{}'s \gls{ebpf} programs and maps into the
kernel, manages their lifecycle, and logs policy enforcement actions for later
examination. This daemon is implemented in bcc~\cite{bcc}, a Python3 library that acts as
...
The sections that follow describe the rule categories supported by
\bpfbox{} (\Crefrange{ss:bpfbox-filesystem-rules}{ss:bpfbox-ptrace-rules}) and the decorators that may
optionally be used to augment them
(\Crefrange{ss:bpfbox-allow}{ss:bpfbox-kfunc}).
\Cref{lst:bpfbox-policy-example} depicts a simple example \bpfbox{} policy.

\begin{lstlisting}[language=bpfbox, gobble=4, caption={An example of a \bpfbox{} policy.}, label={lst:bpfbox-policy-example}, float]
